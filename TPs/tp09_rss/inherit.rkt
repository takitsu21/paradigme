...
(define-type ExpS
  ...
  [instanceofS (expr : ExpS) (class-name : Symbol)])
...
(define-type Exp
  ...
  [instanceofE (expr : Exp) (class-name : Symbol)])
...
(define (exp-s->e [e : ExpS] [super-name : Symbol]) : Exp
  (let ([aux (lambda (e) (exp-s->e e super-name))])
    (type-case ExpS e
      ...
      [(instanceofS expr class-name) (instanceofE (aux expr) class-name)])))
    ...
    (type-case Exp e
      ...
      [(selectE cnd obj arg) (let [(obj-v (interp-r obj))]
                               (type-case Value obj-v
                                 ...
                                 [else (error 'interp "not an object")]))]
      [(instanceofE expr class-name) (begin
                                       (display arg-v)
                                       (instance-aux (interp-r expr) this-v class-name classes))])))

(define (instance-aux expr-v this-v cn classes)
  (type-case Value expr-v
    [(objV class-name fd-vals) (cond
                                 [(equal? 'Object (objV-class-name this-v)) (numV 1)]
                                 [(equal? cn class-name) (numV 1)]
                                 [else (numV 0)])]
    [else (error 'a "not an object")]))

    ...
    [(let/ccE s body)
     (let ([l (new-loc sto)])
       (interp body
               ...
               k))]
    [(beginE l r)
           (interp l env sto-l (beginK ))]
    [(setE par body)
     (let ([l (new-loc sto)])
       (interp body (extend-env (bind par l)) (override-store (cell l (contV k)) sto) (contV k)))]))
...
(define (continue [k : Cont] [val : Value] [sto : Store]) : Value
  (type-case Cont k
    ...
    [(setK v-f next-k) (doAppK v-f next-k)]
    [(beginK v-f r next-k) (interp r env sto (doAppK v-f next-k))]))

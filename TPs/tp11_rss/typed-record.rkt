...
( define ( is-subtype? [t1 : Type ] [t2 : Type ]) : Boolean
   (type-case Type t1
     ((recordT fds t-fds) (type-case Type t2
                            ((recordT fds2 t-fds2) (and
                                                    (is-included fds fds2)
                                                    (check-all-type t-fds fds t-fds2 fds2)))
                                                   
                            (else #f)))
     (else equal? t1 t2)))

(define (check-all-type t-fds1 fds1 t-fds2 fds2)
  (cond
    ((empty? t-fds1) #t)
    ((not (equal? (find (first fds1) fds1 t-fds1) (find (first fds1) fds2 t-fds2))) #f)
    (else (check-all-type (rest t-fds1) (rest fds1) t-fds2 fds2))))

(define (is-included fds1 fds2)
  (cond
    ((empty? fds1 #t))
    ((not (member?

(define (subtype-rec t t-fds)
  (cond
    ((empty? t-fds) #f)
    ((equal? t (first t-fds)) #t)
    (else (subtype-rec t (rest t-fds)))))
;;;;;;;;;;;;;;;;;;;;;;
; Analyse syntaxique ;
;;;;;;;;;;;;;;;;;;;;;;
       ...
       (type-case Type t1
         ((recordT fds t-fds)
          (let ([t-arg (typecheck arg env)])
            ...
            (if (is-subtype? (find fd fds t-fds) t-arg)
                (recordT fds t-fds)
                (type-error-record arg t1))))               
         (else (type-error-record arg t1))))]))
;     (let ([t-rec (typecheck rec env)])
;                         (type-case Type t-rec
;                           ((recordT fds t-fds) (update

; [recordT (fields : (Listof Symbol)) (type-fields : (Listof Type))]
(define (get-aux fd fds types)
  (cond
    [(empty? fds) (error 'get "not found")]
    [(equal? fd (first fds)) (first types)]
    [else (get-aux fd (rest fds) (rest types))]))

(define (set-aux fd fds new-t types)
  (cond
    [(empty? fds) empty]
    [(equal? fd (first fds)) (cons new-t (rest types))]
    [else (cons (first types) (set-aux fd (rest fds) new-t (rest types)))]))
